...
# The library's own DT_SONAME / on-disk name, used only when the symbol table is
# unavailable and the object must otherwise identify itself.
_KEYUTILS_SONAME_MARKERS = (b'libkeyutils.so', b'libkeyutils-')
...
def _verdict(data: bytes) -> Optional[Dict[str, Any]]:
    ...
    #
    # Without the symbol table there is no export/import distinction left to reason from,
    # so the fallback needs a different way to establish that the bytes are libkeyutils at
    # all. Large multi-purpose libraries carry both string sets legitimately: systemd's
    # libsystemd-shared holds keyring helper names and links connect/socket/getaddrinfo,
