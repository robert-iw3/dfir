...
def _matched_string_detail(match, excerpt=48, per_string=4):
    """What each string actually matched, and where in the region.
    ...
    try:
        for s in match.strings:
            ...
            if not isinstance(ident, str):
                ident = (ident or b"").decode("utf-8", "ignore")

            # yara-python >= 4.3 nests hits under each string as `instances`; older builds
            # yield (offset, identifier, data) tuples directly.
            instances = getattr(s, "instances", None)
            if instances is None:
                pairs = ([(s[0], s[2])]
                         if isinstance(s, (list, tuple)) and len(s) >= 3 else [])
            else:
                pairs = [(getattr(i, "offset", 0), getattr(i, "matched_data", b"") or b"")
                         for i in instances]

            for offset, data in pairs[:per_string]:
                clip = data[:excerpt]
                out.append({
                    "id": ident,
                    "offset": int(offset),
                    "text": "".join(chr(c) if 32 <= c < 127 else "." for c in clip),
                    "hex": clip.hex(),
                    "length": len(data),
                })
    except Exception:
    ...
    while True:
        ...
        for m in matches:
            ...
            ent = agg.setdefault(key, {"rule": m.rule, "perms": perms, "region": region,
                                       "path": path, "strings": set(), "matches": [], "n": 0})
            ent["n"] += 1
            # Bounded: a rule with a short or common string can match thousands of times in
            # one region, and the report only needs enough to judge what was found.
            if len(ent["matches"]) < 8:
                ent["matches"].extend(_matched_string_detail(m))
        ...
        if carve_dir and real and carve_this and not carve_all:
            ...
            if p:
                ...
                sys.stderr.flush()
    hits = [{**v, "strings": sorted(v["strings"]), "matches": v["matches"][:8]}
            for v in agg.values()]
