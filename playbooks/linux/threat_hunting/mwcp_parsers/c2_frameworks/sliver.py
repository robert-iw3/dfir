...
# mtls:// and wg:// are Sliver's own transports -- the scheme itself is the evidence, so a
# URL carrying one is this implant's C2 wherever in the region it sits. A plain http(s)/dns
# URL carries no such claim: process memory is full of them, so one counts only when it sits
# beside the config keys that triggered the parser.
_SLIVER_SCHEME_URL_RE = re.compile(rb'(?:mtls|wg)://[^\s\x00\'"<>{}\[\]]{4,200}', re.IGNORECASE)
_GENERIC_URL_RE = re.compile(rb'(?:https?|dns)://[^\s\x00\'"<>{}\[\]]{4,200}', re.IGNORECASE)
...
def extract(data: bytes) -> Optional[Dict[str, Any]]:
    ...
    candidates = [m.group(0) for m in _SLIVER_SCHEME_URL_RE.finditer(data)]
    candidates += values_near(data, _GENERIC_URL_RE, _JSON_ANCHOR)
    for raw in candidates:
        url = decode(raw).rstrip('\x00 /').strip()
