...
# A region this far dominated by prose or source code is documentation, a detection rule, an
# analyst's notes or an editor buffer -- not a packed structure a program parses at runtime.
# Measured: every synthetic TP sample scores 0.00 and every parser source scores 0.96-1.00,
# so the bar sits far from both.
_PROSE_SUPPRESSION_THRESHOLD = 0.60


def prose_fraction(data: bytes) -> float:
    """Share of the buffer sitting inside whitespace-bearing printable runs."""
    if not data:
        return 0.0
    total = 0
...
def extract_all(data: bytes) -> List[Dict[str, Any]]:
    ...
    One thing IS suppressed: a region that is overwhelmingly human-readable text. Parsers
    key on literals, and a buffer describing a family contains every literal that family is
    identified by -- documentation, a rule set, this catalog's own source. Reporting those
    as recovered configuration asserts an implant from a description of one, and on a real
    workstation capture it did exactly that. The gate is here rather than in each parser so
    it also covers families added later.
    """
    if not data:
        return []
    if prose_fraction(data) >= _PROSE_SUPPRESSION_THRESHOLD:
        return []
    hits = []
