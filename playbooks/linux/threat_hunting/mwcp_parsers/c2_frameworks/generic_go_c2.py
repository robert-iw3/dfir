...
def extract(data: bytes) -> Optional[Dict[str, Any]]:
    ...
    for obj in find_json_objects(data, _HEARTBEAT_JSON_ANCHOR):
        fields.update(obj)
    # This parser names no family, so its whole claim rests on the heartbeat JSON sitting
    # next to an endpoint. Every URL a Go process holds is not a candidate C2 -- only the
    # ones beside that JSON are what the shape argument is actually about.
    urls = sorted({decode(u) for u in values_near(data, _URL_RE, _HEARTBEAT_JSON_ANCHOR)})
