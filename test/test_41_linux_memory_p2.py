...
These verify the FP-calibration proven against a live workstation image: raw
...
def test_bash_benign_tmp_references_not_flagged():
    """The exact FP set from a live workstation run: bare /tmp references are benign."""
    rows = [{"Command": c, "PID": 72987} for c in
            ("ls /tmp/build-cache/", "cd /tmp/", "cat /tmp/test_run_phase4_zero3.txt",
             "rm -rf /tmp/build-cache/", "tail -f /tmp/k8s_run.log")]
