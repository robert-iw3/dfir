...
- Run: `uat_audit.sh` — 2026-08-17 21:50:06Z

**Sign-on — a login the platform was never told about**

| Result | Assertion — with evidence |
|---|---|
| · | before: 903 login entries, 43052 entries in total |
...
| ✅ PASS | the sign-on produced exactly ONE user.login entry (903 → 904) across a login plus 5 further requests |
...
| ✅ PASS | the login entry records where the analyst connected from and what they used (10.89.1.143, Mozilla/5.0 (X11; Linux …) |

**Writes — recorded whether or not anyone instrumented the route**

| Result | Assertion — with evidence |
|---|---|
| ✅ PASS | an edit on an uninstrumented route succeeded (HTTP 200 on note 14758) — the case the catch-all exists for |
| ✅ PASS | the write was recorded with its verb, route and outcome: notes.modify by uat-audit-probe (PATCH /api/notes/14758/ → 200) |
| ✅ PASS | the explicitly audited create produced its own entry only — the catch-all added no duplicate (notes.create rows: 0, alongside 14719 note.create) |

**Sign-out — sessions that end, and sessions nobody closed**

| Result | Assertion — with evidence |
|---|---|
| ✅ PASS | the analyst signed out through the platform (HTTP 200, session 906) |
| ✅ PASS | the sign-out closed an OPEN sign-on rather than reporting a no-op |
| ✅ PASS | user.logout entries exist in the trail (45) — a session end is an audited event, not a gap |
| ✅ PASS | the sign-on record closed with a reason and a request count: uat-audit-probe ended by signout after 1 requests |
| ✅ PASS | a sign-on idle past the cookie lifetime is closed as expired (211 closed on that sweep) — abandoned sessions do not stay open forever |
...
| ✅ PASS | every sign-on/sign-out entry is HMAC-signed (1799/1799) — the same protection as the rest of the trail |

**Coverage — the actions an auditor is entitled to ask about**

| Result | Assertion — with evidence |
|---|---|
| · | recorded: login=904\|logout=45\|create=15885\|modify=14738\|delete=31\|export=4448 |
| ✅ PASS | login is represented in the trail (904 entries) |
| ✅ PASS | logout is represented in the trail (45 entries) |
| ✅ PASS | create is represented in the trail (15885 entries) |
| ✅ PASS | export is represented in the trail (4448 entries) |

**Attribution — a session belongs to a person, or says it cannot tell**

| Result | Assertion — with evidence |
|---|---|
| ✅ PASS | every brokered session carries an attribution verdict (34/34; kinds seen: exact,none) |
| ✅ PASS | sessions reported as exact carry a named analyst (8 of 34 named overall) |
