...
- Run: `uat_load.sh` — 2026-08-15 01:15:30Z
...
| Result | Assertion — with evidence |
|---|---|
| ✅ PASS | all 50 users provisioned concurrently (p95 786ms per create) |
...
| ✅ PASS | storm p95 1849ms over 50 completed logins, within the 8000ms ceiling (max 2270ms) |
...
| ✅ PASS | read_stats: p95 225ms within 1500ms under 50-wide contention |
| ✅ PASS | read_findings: p95 134ms within 1500ms under 50-wide contention |
| ✅ PASS | write_note: p95 298ms within 2500ms while colliding on one investigation |
| ✅ PASS | write_verdict: p95 581ms within 2500ms while colliding on one investigation |
| ✅ PASS | the application raised nothing across 4728 operations — every loss below is transport, not logic |
| ✅ PASS | 37 of 4728 operations (0%) lost their connection, within the 3% a brokered path allows — 28 session replacement(s) account for them, and each costs only its own share |
| ✅ PASS | RBAC under load: all 247 auditor writes that reached authorization were refused 403 (2 never served) |
...
| ✅ PASS | confidentiality: zero identity-bleed or privilege violations across 1773 concurrent checks (3 requests never served, excluded) |
| ✅ PASS | export boundary exercised both ways under load (40 completed, 250 refused) |
| ✅ PASS | data fidelity: 983 notes for 981 the agents counted as accepted — 2 committed with the response lost on the way back, which loses nothing |
| ✅ PASS | data fidelity: every one of 986 accepted adjudications is in the database by id, and the database holds no others |
| ✅ PASS | the export ledger accounts for every attempt under load (40 completed, 250 denied — matching the agents exactly) |
...
| ✅ PASS | availability 99.54% over 216 independent 1 Hz samples (health p95 5ms) — floor 99% |
