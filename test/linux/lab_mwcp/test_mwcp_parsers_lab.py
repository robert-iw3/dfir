...
# A recovered endpoint has to come from the structure that triggered the parser. Process
# memory holds every URL the process ever touched, so a parser that falls back to "any URL
# in the region" names whichever unrelated host sorts first as C2 infrastructure.
class TestRecoveredEndpointsAreCoLocated:
    # Well past the co-location window, so nothing ties this URL to the config above it.
    _FAR_AWAY = os.urandom(4000) + b'\x00https://unrelated-cdn.example.com/asset.js\x00'

    def test_generic_go_c2_ignores_distant_urls(self):
        blob = (b'Go build ID: "abc"\x00runtime.goexit\x00'
                b'{"hostname": "victim", "interval": 30, "task_id": "t1"}\x00'
                + self._FAR_AWAY)
        result = generic_go_c2.extract(blob)
        assert result is not None
        assert 'https://unrelated-cdn.example.com/asset.js' not in result['candidate_urls']

    def test_merlin_ignores_distant_urls(self):
        blob = b'{"psk": "s", "maxRetry": 7, "proto": "h2"}\x00' + self._FAR_AWAY
        result = merlin.extract(blob)
        assert result is None or 'unrelated-cdn' not in str(result.get('c2_url', ''))

    def test_sliver_keeps_its_own_transport_schemes(self):
        """mtls:// and wg:// are Sliver's own transports, so the scheme itself carries the
        claim and the URL counts wherever it sits."""
        blob = (b'{"implant_name": "abc", "reconnect_interval": 60}\x00'
                + os.urandom(3000) + b'\x00mtls://10.0.0.5:8888\x00')
        result = sliver.extract(blob)
        assert result is not None
        assert any('mtls://' in u for u in result['c2_urls'])

    def test_sliver_ignores_distant_plain_urls(self):
        blob = (b'{"implant_name": "abc", "reconnect_interval": 60}\x00'
                b'"c2s"\x00mtls://10.0.0.5:8888\x00' + self._FAR_AWAY)
        result = sliver.extract(blob)
        assert result is not None
        assert not any('unrelated-cdn' in u for u in result['c2_urls'])
...
class TestEburyCapabilityMismatchFallback:
    def test_detects_capability_mismatch(self):
        blob = (b'libkeyutils.so.1\x00keyctl\x00add_key\x00request_key\x00'
    ...
    def test_multipurpose_library_strings_are_not_the_claim(self):
        """Without a parseable symbol table there is no export/import relationship left
        to reason from, so the bytes must at least claim to be libkeyutils. A large
        general-purpose library carries keyring helper names and links the network
        primitives legitimately, and being truncated past its symbol table -- by a
        caller's read cap, or by a partial carve -- is what routes it down this path."""
        blob = (b'libsystemd-shared-257.so\x00keyctl\x00add_key\x00request_key\x00'
                b'connect\x00getaddrinfo\x00socket\x00')
        assert ebury.identify(blob) is False
...
def test_multiple_families_can_coexist():
    blob = (_make_mirai_table() + os.urandom(200) +
           b'libkeyutils.so.1\x00keyctl\x00add_key\x00request_key\x00'
           b'connect\x00socket\x00getaddrinfo\x00')
