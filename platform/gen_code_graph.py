...
def compose_topology():
    ...
    tiers = {}
    for tier in TIERS:
        files = [ROOT / "deploy" / tier / "docker-compose.yml"]
        mh = ROOT / "deploy" / tier / "docker-compose.multihost.yml"
        if mh.exists():
            files.append(mh)
        services = {}
        for f in files:
...
def script_graph():
    ...
    scripts = {}
    all_sh = {p.name: p.relative_to(ROOT).as_posix()
...
def uat_coverage():
    ...
    cover = {}
    for p in sorted((ROOT / "test").glob("uat_*.sh")):
