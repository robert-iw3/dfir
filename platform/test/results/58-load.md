...
- Run: `uat_load.sh` — 2026-08-12 14:11:05Z

**Preflight — targets, snapshots, samplers**

| Result | Assertion — with evidence |
|---|---|
| ✅ PASS | contention targets: investigation 4, findings 629,630,631,632,633 (verdicts snapshotted for restore) |
...
| Result | Assertion — with evidence |
|---|---|
| ✅ PASS | all 50 users provisioned concurrently (p95 439ms per create) |
...
| ✅ PASS | storm p95 2469ms over 50 completed logins, within the 8000ms ceiling (max 2939ms) |
| ✅ PASS | 50 DISTINCT server-side sessions created (redis db1 57 -> 108) |
...
| ✅ PASS | read_stats: p95 187ms within 1500ms under 50-wide contention |
| ✅ PASS | read_findings: p95 133ms within 1500ms under 50-wide contention |
| ✅ PASS | write_note: p95 403ms within 2500ms while colliding on one investigation |
| ✅ PASS | write_verdict: p95 747ms within 2500ms while colliding on one investigation |
| ✅ PASS | the application raised nothing across 5107 operations — every loss below is transport, not logic |
| ✅ PASS | 76 of 5107 operations (1%) lost their connection, within the 3% a brokered path allows — 13 session replacement(s) account for them, and each costs only its own share |
| ✅ PASS | RBAC under load: all 275 auditor writes that reached authorization were refused 403 (1 never served) |
...
| ✅ PASS | confidentiality: zero identity-bleed or privilege violations across 1872 concurrent checks (3 requests never served, excluded) |
| ✅ PASS | export boundary exercised both ways under load (40 completed, 259 refused) |
| ✅ PASS | data fidelity: 1021 notes in the database — exactly the 1021 the agents recorded as accepted, none lost, none duplicated |
| ✅ PASS | data fidelity: 1022 reclassification history rows — one per accepted adjudication under contention |
| ✅ PASS | the export ledger accounts for every attempt under load (40 completed, 259 denied — matching the agents exactly) |
...
| ✅ PASS | availability 100.00% over 215 independent 1 Hz samples (health p95 3ms) — floor 99% |
