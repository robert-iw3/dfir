...
# Mining-pool indicators. Every alternative has to be something only a miner's command line
# carries, and a bare `pool.` is not: it matches inside `ForkJoinPool.common`, `ThreadPool.`,
# `ConnectionPool.` and any JVM or .NET stack trace. It flagged this platform's own Keycloak
# as a cryptominer at High confidence, Likely True Positive.
#
# A pool reference must therefore look like a HOSTNAME rather than a fragment: `pool.` followed
# by a domain and a TLD. `pool.minexmr.com` still matches, `ForkJoinPool.common` no longer can.
# The flag and URL-scheme forms stay as they were — a command line carrying `stratum+tcp://` or
# `--donate-level` is not doing anything else.
POOL_RE = re.compile(
    r"stratum\+(?:tcp|ssl)://"
    r"|--pool\b"
    r"|--donate-level\b"
    r"|\bpool\.[a-z0-9][a-z0-9-]*(?:\.[a-z0-9-]+)*\.[a-z]{2,}\b"
    r"|\bxmr\.[a-z0-9][a-z0-9-]*(?:\.[a-z0-9-]+)*\.[a-z]{2,}\b"
    r"|\bminingpool\b",
    re.I)
