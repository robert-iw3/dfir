...
# IR_HOSTNAME names the machine the evidence belongs to. It matters wherever the shell's own
# hostname is not that machine's: inside a container (where it is the container id), or when
# collecting against a mounted image. Every artifact, finding and status record is keyed to
# this value, so a wrong one files a host's evidence under a name nothing else will match.
HOSTNAME_S="${IR_HOSTNAME:-$(hostname -s 2>/dev/null || hostname 2>/dev/null || echo unknown)}"
